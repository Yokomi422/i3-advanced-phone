...
void decrypt_message(char *message, int len) {
    for (int i = 0; i < len; ++i) {
        message[i] ^= 0xAA;  // Simple XOR decryption
    }
}

void encrypt_message(char *message, int len) {
    for (int i = 0; i < len; ++i) {
        message[i] ^= 0xAA;  // Simple XOR encryption
    }
}
...
int main(int argc, char **argv) {
    ...
    while (1) {
        ...
        if (fds[0].revents) {
            ...
            if (recv_size > 0) {
                decrypt_message(recv_buf, recv_size);
        ...
        if (fds[1].revents) {
            ...
            if (send_size > 0) {
                encrypt_message(send_buf, send_size);
